...
def fetch_household_income(zipcode):
    svg = fetch_svg(STAT_ATLAS_HOUSEHOLD_INCOME_URL, zipcode, 'figure/household-income-percentiles')
    gs = svg.g.find_all('g')
    result = [
        gs[8],
        gs[10],
...
def fetch_household_income_distribution(zipcode):
    svg = fetch_svg(STAT_ATLAS_HOUSEHOLD_INCOME_URL, zipcode, 'figure/household-income-distribution')
    print(svg)
    gs = svg.g.find_all('g')
    result = []
    for x in range(len(gs)):
...
def fill_zip_worksheet(workbook, zipcode):
    ...
    write_labeled_data(worksheet, 'Population by Age Segment', AGE_SEGMENT_LABELS, age_segments, 4)
    write_labeled_data(worksheet, 'Household by Type', HOUSEHOLD_TYPE_LABELS, household_type, len(age_segments) + 6)
    write_labeled_data(worksheet, 'Income Distribution', INCOME_DIST_LABELS, income_dist, len(age_segments) + len(household_type) + 8)
...
def fill_computation_tab(worksheet, zipcodes):
    ...
    for income_group in INCOME_GROUPS:
        ...
        for x in range(start_x, len(INCOME_DIST_LEVELS)):
            for zip in zipcodes:
                tab_name = ZIP_DATA_SHEET_NAME % zip
                numerator.append("('%s'!B%d*'%s'!B1)" % (tab_name, INCOME_DIST_LEVELS[x][1], tab_name))
