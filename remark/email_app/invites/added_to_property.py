...
from remark.email_app.constants import SENDGRID_SENDER_FROM
from remark.lib.sendgrid_email import send_email


@shared_task
def send_invite_email(user_id, projects_ids, max_count=5):
    ...
    for p in projects[:max_count]:
        ...
        properties.append({
            "image_url": p.get_building_image()[1],
            "title": p.name,
    ...
    send_email(
        from_email=SENDGRID_SENDER_FROM,
